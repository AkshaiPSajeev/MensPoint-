...
router.get('/place-order',verifyLogin,async(req,res)=>{
  ...
  res.render('user/place-order',{products:products,addresses:addresses,user:req.session.user,userId:req.session.user._id,orderSummary:orderSummary,coupons:coupons});
})
...
router.post('/place-order',verifyLogin,async(req,res)=>{
  ...
  let addressObj={
   ...
   req.body.AddressId=addressId;
     ...
     userHelpers.placeOrder(req.body.userId,req.body).then(async(orderId)=>{
      ...
      //here 1 is meant to be total of ie order amount
       userHelpers.generateRazorpay(orderId,total).then((razobj)=>{
      ...
      userHelpers.generatePaypal(orderId,total).then((paypalobj)=>{
        let response={
          //status:false,
          cod:false,
         paypal:true,
   ...
   emailHelpers.sendEmail(req.session.user._id,path);
...
router.get('/edit-user-details',verifyLogin,(req,res)=>{
  res.render('user/edit-user',{userDetails:req.session.user,user:req.session.user})
})
...
router.get('/address-management',verifyLogin,(req,res)=>{
  userHelpers.getUserAddresses(req.session.user._id).then((addresses)=>{
    res.render('user/address-management',{user:req.session.user,addresses})
  })
...
router.get('/change-password',verifyLogin,(req,res)=>{
  ...
  res.render('user/change-password',{user:req.session.user,changePasswordStatus:changePasswordStatus})
...
router.get('/offers-coupons',verifyLogin,async(req,res)=>{
  ...
  res.render('user/offers-coupons',{user:req.session.user,coupons:coupons});
})
...
router.get('/view-orderitems',verifyLogin,(req,res)=>{
  ...
  userHelpers.viewOrderedItems(req.query.orderId).then((items)=>{
    ...
    res.render('user/view-order-items',{user:req.session.user,items})
  })
